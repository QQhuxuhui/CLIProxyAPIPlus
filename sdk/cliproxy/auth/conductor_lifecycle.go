...
func (m *Manager) updateInternal(ctx context.Context, base, auth *Auth, mode updateAuthMode) (*Auth, error) {
	...
	invalidatedCredentialsChanged := false
	if CredentialsChanged(existing, auth) {
		// Fresh credential material resets the consecutive-401 counter and lifts
		// a codex credential-invalidated park so the new tokens get probed.
		auth.AuthFailures = 0
		if codexCredentialInvalidated(existing) {
			invalidatedCredentialsChanged = true
			...
			// Drop the per-model 401 states that parked the credential; other
			// model states (quota cooldowns etc.) are unrelated to the login and
			// are kept.
			source := auth.ModelStates
			if len(source) == 0 {
				source = existing.ModelStates
			}
			auth.ModelStates = codexModelStatesWithoutUnauthorized(source)
			clearUnauthorizedModelStates(auth, time.Now())
		}
	} else {
	...
	cooldownStateChanged := invalidatedCredentialsChanged
