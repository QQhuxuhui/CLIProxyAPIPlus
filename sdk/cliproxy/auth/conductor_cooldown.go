				...
				if !shouldSkipCredentialCooldown(result.Error) {
					...
					if isModelSupportResultError(result.Error) {
						...
						case 429:
							...
							explicitQuotaReset = explicitReset
							if reasonCode == "" {
							...
							if !disableCooling {
								...
								if !explicitReset && prevQuota.Exceeded && prevQuota.NextRecoverAt.After(next) {
						...
						case 0:
							// Status-less transport failures are benched like any other
							// transient upstream fault, except when the downstream client
							// simply went away: a canceled request says nothing about the
							// credential's health.
							if isCanceledResultError(result.Error) {
								state.NextRetryAfter = time.Time{}
								state.Unavailable = false
							} else {
					...
					// clears the deadline.
					if !explicitQuotaReset && !state.NextRetryAfter.IsZero() && prevModelRetryAfter.After(state.NextRetryAfter) && prevModelRetryAfter.After(now) {
...
func mergeModelState(target, source *ModelState) *ModelState {
	...
	merged := ModelState{
		...
		Quota: QuotaState{
			...
			ResetAt:       preferred.Quota.ResetAt,
			ReasonCode:    preferred.Quota.ReasonCode,
			UpstreamModel: preferred.Quota.UpstreamModel,
		},
...
func applyAuthFailureState(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time, disableCooling bool) {
	...
	} else if isInvalidGrantResultError(resultErr) && statusCode != http.StatusUnauthorized {
