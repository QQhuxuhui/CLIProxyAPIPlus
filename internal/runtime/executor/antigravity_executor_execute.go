...
func (e *AntigravityExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	releaseSession, errAcquire := e.sessionLocks().Acquire(ctx, antigravitySessionLockKeyFor(ctx, req, opts, helps.DerivedAntigravitySessionID(opts.Metadata, req.Metadata)))
	if errAcquire != nil {
	...
	if errVersion := antigravityHandleVersionError(ctx, auth, baseModel, bodyBytes); errVersion != nil {
		err = errVersion
		return resp, err
	}
	// Safety blocks also arrive as 2xx: empty candidates or finishReason=SAFETY
	// without any parts. Translating that yields a zero-token "success" the
	// downstream cannot tell apart, so surface an explicit 400 content_filter.
	// No cooldown and no credential rotation: the block is decided by content.
...
func (e *AntigravityExecutor) executeClaudeNonStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	go func(resp *http.Response) {
		...
		for scanner.Scan() {
			...
			// A safety block before any content becomes an explicit 400 for the
			// whole (buffered) response, mirroring ExecuteStream.
