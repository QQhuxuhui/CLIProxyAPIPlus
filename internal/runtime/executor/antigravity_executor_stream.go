...
func (e *AntigravityExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	go func(resp *http.Response) {
		...
		var param any
		// Only convert a safety block into an error before any generated content
		// has reached the downstream; afterwards the terminal chunk's finishReason
		// carries it, and erroring would merely truncate.
		...
		for scanner.Scan() {
			...
			// but benching the account protects later requests and ending with an
			// error avoids a zero-token success downstream.
