...
func TestAntigravityReasoningReplayClearsOnInvalidSignature400(t *testing.T) {
	...
	// Replay session keys are scoped to the authenticated caller; seed and assert
	// under the scoped key and drive Execute with the matching caller context.
	sessionKey := scopedReplayKey("session:pr3900-invalid-sig")
