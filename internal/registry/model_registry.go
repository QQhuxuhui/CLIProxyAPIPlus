...
//   - resetAt: The absolute time at which the model quota resets (the recovery time).
...
func (r *ModelRegistry) SetModelQuotaExceededUntil(clientID, modelID string, resetAt time.Time) {
	...
	if registration, exists := r.models[modelID]; exists {
		now := time.Now()
		if resetAt.IsZero() {
			resetAt = now.Add(modelQuotaExceededWindow)
		}
...
type ClientModelProjection struct {
	...
	QuotaExceeded bool
	// QuotaRecoverAt is the absolute time the quota block is expected to lift.
	// Zero falls back to now+modelQuotaExceededWindow.
...
func (r *ModelRegistry) ApplyClientModelProjections(clientID string, epoch uint64, generation uint64, projections []ClientModelProjection) bool {
	...
	for _, proj := range projections {
		...
		if proj.QuotaExceeded {
			...
			recoverAt := proj.QuotaRecoverAt
			if recoverAt.IsZero() || !recoverAt.After(now) {
				recoverAt = now.Add(modelQuotaExceededWindow)
			}
			if existing, ok := registration.QuotaExceededClients[clientID]; !ok || existing == nil || !existing.Equal(recoverAt) {
...
func modelRegistrationAvailability(registration *ModelRegistration, now time.Time) (bool, time.Time) {
	...
	for _, quotaTime := range registration.QuotaExceededClients {
		...
		// QuotaExceededClients stores the absolute recovery time.
		recoveryAt := *quotaTime
		...
		if ok && registration != nil {
			if registration.QuotaExceededClients != nil {
				for clientID, quotaTime := range registration.QuotaExceededClients {
					...
					if quotaTime != nil && now.Before(*quotaTime) {
						expiredClients++
					}
				}
			}
